"""
This is an example of using the networkx library for the networks project for C&N_0
...
# Will only print out information if N_0 is below this number, use for testing
Nprint = 20
...
printon = False
if (N < Nprint):
    printon = True  # print stuff when debugging with small N_0
...
# This section add N_0 nodes.
# Each node has unique number starting with 0 up to (N_0-1) if there are N_0 nodes
for i in range(N):
    G.add_node(i)
# Could have used
...
for s in range(N):
    for t in range(s + 1, N):
        if (random.random() < p):
            G.add_edge(s, t)
            if printon:
                print('--- new edge from ' + str(s) + ' to ' + str(t))  # useful for debugging
...
# My preferred network software is Visone and this format works best for Visone
# nx.write_graphml(G, outputfile) # write out .graphml file, comment out if don't want this

# For Windows users, pajek is a common network package but its not easy to use
# However, almost all packages can read the .net fiel format used by pajek
# e.g. Visone or gephi
